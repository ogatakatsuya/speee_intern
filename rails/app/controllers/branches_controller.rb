...
class BranchesController < ApplicationController
  ...
  def show
    ...
    sales_satisfaction_range = max_sales_satisfaction - min_sales_satisfaction
    scaled_sales_satisfaction = (((branch_reviews.min_sales_satisfaction_indicator - min_sales_satisfaction) * 4) / sales_satisfaction_range) # rubocop:disable Layout/LineLength
    @normalized_sales_satisfaction = 6 - (1 + scaled_sales_satisfaction)
    ...
    # 差があればあるほど値が小さくなるように反転させている
    sales_speed_range = max_sales_speed - min_sales_speed
    scaled_sales_speed = (((branch_reviews.min_sales_speed_indicator - min_sales_speed) * 4) / sales_speed_range)
    @normalized_sales_speed = 6 - (1 + scaled_sales_speed)
  end
end
